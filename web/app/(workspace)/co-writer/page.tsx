...
export default function CoWriterHomePage() {
  ...
  const renderEmpty = () => (
    <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-dashed border-[var(--border)] bg-[var(--secondary)]/30 px-8 py-16 text-center">
      ...
      <div>
        <p className="text-base font-medium text-[var(--foreground)]">{t('No drafts yet')}</p>
    ...
    <div className="flex h-full min-h-full flex-col overflow-hidden bg-[var(--background)]">
      <header className="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-6 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-[var(--foreground)]">{t('Co-Writer')}</div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      ...
                      onClick={event => {
                        ...
                        if (window.confirm(t('Delete this draft?'))) {
                          void handleDelete(doc.id)
                        }
                      }}
                      disabled={isDeleting}
                      title={t('Delete draft')}
                      className="shrink-0 rounded-md p-1 transition-colors disabled:opacity-50 text-[var(--muted-foreground)]/60 opacity-0 hover:bg-rose-500/10 hover:text-rose-600 group-hover:opacity-100 dark:hover:text-rose-400"
                    >
                      {isDeleting ? (
